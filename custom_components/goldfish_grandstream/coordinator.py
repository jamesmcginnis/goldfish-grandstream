...
# How many consecutive failures before we give up and go unavailable
MAX_FAILURES = 3
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        try:
            ...
            if self._consecutive_failures >= MAX_FAILURES:
                # Only go unavailable after several consecutive failures
                raise UpdateFailed(
                    f"Error communicating with {self.host} "
                    f"({self._consecutive_failures} consecutive failures): {err}"
                ) from err

            # Return last known data so the entity doesn't flicker to unavailable
            # during a brief blip (e.g. phone busy during a call)
            _LOGGER.debug(
