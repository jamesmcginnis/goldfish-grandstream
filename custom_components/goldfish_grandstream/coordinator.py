...
class GrandstreamCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        try:
            # Refresh the session so the sid doesn't expire between polls
            await self._client.refresh_session()

            status = await self._client.get_phone_status()
        except GrandstreamAuthError as err:
            raise UpdateFailed(f"Authentication error with {self.host}: {err}") from err
