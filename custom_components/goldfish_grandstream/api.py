...
class GrandstreamApiClient:
    ...
    async def get_phone_status(self, *, _retried: bool = False) -> dict[str, Any]:
        ...
        # The phone returns body="unauthorized" when the session has expired
        # (it does not use a non-success response or HTTP 401 for this).
        # Treat it as an auth failure and re-authenticate.
        if raw_status == "unauthorized":
            if _retried:
                raise GrandstreamConnectionError("Session expired and re-auth failed")
            _LOGGER.debug("Session expired (body=unauthorized), re-authenticating")
            self._authenticated = False
            await self.authenticate()
            return await self.get_phone_status(_retried=True)
